...
`OMEinsumContractionOrders` (One More Einsum Contraction Orders, or OMECO) is a Julia [@bezanson2012julia] package that implements state-of-the-art algorithms for optimizing tensor network contraction orders.
OMECO is designed to search for near-optimal contraction orders for exact tensor network contraction. It provides a comprehensive suite of optimization algorithms for tensor network contraction orders, including greedy heuristics, simulated annealing, and tree-width solvers.
This paper presents the key features of OMECO, its integration with the Julia ecosystem, and performance benchmarks.
...
More real-world examples demonstrating applications to quantum circuit simulation, combinatorial optimization, probabilistic inference, and quantum error correction are available in the [OMEinsumContractionOrdersBenchmark](https://github.com/TensorBFS/OMEinsumContractionOrdersBenchmark) repository. We find that optimizer performance is highly problem-dependent, with no single algorithm dominating across all metrics and graph topologies.
